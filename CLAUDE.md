...
- **Backend** (`Server/`) — FastAPI；除 `Server/artcn.db` 外无状态：
  - `main.py` — routes, lifespan (storage cleanup, worker start), error handlers, static hosting; 统一鉴权中间件 + auth/admin 路由（Spec2）；`ThreadStore`（内存路由上下文，启动时按 `conversation_id` 从 `chat_messages` 回填）。鉴权中间件顺带判服务限额（Spec18，超额 40304）——它每个请求实时查库，所以改限额**立刻**生效。
  - `db.py` — SQLite 账号库（users 表 CRUD、首次启动自动建初始管理员）+ Spec17 的 `conversations` / `chat_messages` / `post_comments` 与 `app_meta` 键值 + Spec18 的 `user_quota`（计费账户）。`record_call` **双写** `usage`（区间统计）+ `user_quota`（计费账户），同一事务（Spec18 §2.2）。
...
- **`Server/config.py`** is the single source of env config, read from **`Server/.env`** via python-dotenv (`.env.example` has placeholders). Real keys never go in the repo.
  ...
  - 服务限额（Spec18）: `QUOTA_DEFAULT_LIMIT`（默认 25）、`QUOTA_LIMIT_MAX`（默认 100000，管理员可设上限）、`QUOTA_CONTACT_EMAIL`（默认 frostyj@qq.com，拼进 `QUOTA_EXCEEDED_MESSAGE`，**前端零副本**——只显示后端回传的 `message`）。**`QUOTA_DEFAULT_LIMIT` 只影响新建用户**（补列迁移时给存量行填初值也用它），改它不会给任何已存在的账号加额度——那要走管理端「用户管理」页的「改限额」（`PUT /api/admin/users/{id}/quota`）。
...
- **帖子弹窗的 flex 塌陷陷阱**（Spec17 §14.1）：`.post-comments` 是 `.community-modal-body`（flex 列）的子项且**自身是滚动容器**，`min-height: auto` 会解析成 0，于是它**先于**别的元素被压到 0 高——曾经导致带图帖的评论区整个消失（`comment-form` 不是滚动容器，压不动，所以只有评论区让位）。保命的是那行 `min-height: 110px`，别删；改动弹窗高度分配时（图片 `max-height: 40vh`、固定内容 `flex-shrink: 0`）要一并考虑。
...
- **`usage` 与 `user_quota` 是两个口径**（Spec18 §2.2）：`usage` 可被「清空调用统计」删行归零（**区间统计**），`user_quota.used` 只增不减（**计费账户**）。清零后前者恒小于后者——这是有意的，清空统计**不是退钱**，超额用户不会被解封。别把 `used` 改成"现算 `usage` 五列之和"。
- **`record_call` 必须一次事务写两张表**（Spec18 §2.2）：不要在调用方串两次，也不要拆成两个函数——那会开出一个"统计涨了但没扣费"的窗口。
- **风格归纳的计数埋点在 `await` 之前**（Spec18 §5.2B）：`wiki.py` 的两处 `db.record_call(user_id, "style")` 记的是"调用发出"，与四类任务的"成功才计"不同——上游失败时钱已经花了，所以照计。别顺手改成成功后才记。四类任务的计数时机/条件一个字没变。
