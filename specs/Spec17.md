...
| 决策 | 选择 | 理由 |
|---|---|---|
| 图片高度 | 上限从 `60vh` 收到 **`45vh`** | 图片是弹窗里唯一"多高都合理"的元素，让它先让位，固定内容（作者行 / 全文 / 投票行 / 输入框）才有余量 |
...
| 是否给配图加"点击放大" | **不加** | 作者选的是"限图 + 评论保底"方案；图片限到 45vh 后仍完整可辨，再加一层 lightbox 是新的交互面（§3 的 YAGNI 立场一致） |
...
| `.community-modal-img` | `max-height: 60vh → 45vh` | 图片不再独占高度 |
...
| # | 操作 | 期望 |
|---|---|---|
| 61 | 打开一条**带图**且有评论的帖子 | 图片 ≤`45vh`；评论区可见（≥110px），评论与「发表」输入框**同屏** |
| 62 | 把窗口压到 768px 高，重复 #61 | 评论区仍保有 110px 可滚动区，不被图片与输入框挤成 0 高 |
