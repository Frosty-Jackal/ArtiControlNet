...
首轮验收发现「带图帖子即使有评论也看不见评论区」（纯文字帖正常）。根因不是"空间不够"，而是 flex 压缩顺序，详见 §14.1。

| 决策 | 选择 | 理由 |
|---|---|---|
| 图片高度 | 上限从 `60vh` 收到 **`40vh`** | 图片是弹窗里唯一"多高都合理"的元素，让它先让位，固定内容（作者行 / 全文 / 投票行 / 输入框）才有余量。**40vh 是实测定下的**：图片 `0.40vh` + 正文固定 `301px` ≤ 弹窗 `0.92vh` ⟺ 视口高 ≥ 580px，即 768px 窗口（视口 668）下评论与输入框同屏；`45vh` 在那里差 33px，输入框掉到折叠线以下 |
| 评论区高度 | `max-height` 200 → **260px**，并加 **`min-height: 110px`** | 「固定高内滚动、不撑高弹窗」的既有约定（§7.7 / 验收 #53）不变——只是补下限。`min-height` 优先于 `flex-shrink`，这是"永不被压没"的落点 |
| 弹窗尺寸 | `max-width` 720 → **760px**，`max-height` 90% → **92%** | 与上面两条配套的整体余量 |
| 是否给配图加"点击放大" | **不加** | 作者选的是"限图 + 评论保底"方案；图片限到 40vh 后仍完整可辨（800px 视口下 320px，`object-fit: contain` 不裁切），再加一层 lightbox 是新的交互面（§3 的 YAGNI 立场一致） |

> 注：§7.7 把评论区画在「卡片底部，内联」，落地时放在**帖子弹窗**内（`💬 评论 N` 计数在投票行右侧）。理由是瀑布流卡片本身是 `<button>`，装不下 `input`；且纯文字帖不渲染缩略图，卡片确实变矮了，§7.7 的两个诉求都已覆盖。§14.1 的布局修订适用于**弹窗内**的评论区。
...
## 14. 补充（2026-09-15 验收后修订）

首轮实现提交（`53c3ddf`）后，作者在浏览器里发现两处问题。本节记录修法与新增验收用例；决策理由见 §2.4。

### 14.1 帖子弹窗：带图帖的评论区被压成 0 高（Bug 修复）

**现象**：带图帖子即使有评论，弹窗里也看不到评论区（只能看到图片和「写评论…」输入框）；纯文字帖一切正常。

**根因**——不是"空间不够"，是 flex 压缩顺序：

`.post-comments` 是 `.community-modal-body`（`display: flex; flex-direction: column`）的子项，`flex-shrink` 默认 `1`；而它自身是滚动容器（`overflow-y: auto`）。按规范，滚动容器的 `min-height: auto` 解析为 **0**——于是当内容超出弹窗高度时，它**先于**其他元素被压到 0 高。

对照之下 `.comment-form` 不是滚动容器，`min-height: auto` = 内容高，**压不动**。所以带图帖里图片占满 `60vh` 后，被牺牲的永远是评论列表，而不是让弹窗本体出现滚动条——用户看到的就是"输入框在、评论不见"。纯文字帖没有图片占位，高度宽裕，评论区按内容高正常显示。

**修法**（五处同时改，缺一不可；除第一处外都在 `main.css`）：

| 位置 | 改动 | 目的 |
|---|---|---|
| `.community-modal`（`CommunityPanel.vue` scoped） | `max-width: 720px → 760px`；`max-height: 90% → 92%` | 整体余量 |
| `.community-modal-img` | `max-height: 60vh → 40vh` | 图片不再独占高度 |
| `.post-comments` | 加 `flex: 1 1 auto; min-height: 110px`；`max-height: 200px → 260px` | **保底可见**：`min-height` 优先于 `flex-shrink`，任何窗口高度下都留出可滚动区 |
| `.community-modal-head` / `.community-modal-text` / `.community-modal-actions` / `.comment-form` | 加 `flex-shrink: 0` | 固定内容显式声明不被压缩（它们本来就压不动，写明以防日后被误改） |
| `.community-modal-text` | 加 `max-height: 22vh; overflow-y: auto` | 帖子文字上限 1000 字（约 30 行）时不独占弹窗 |

只放大弹窗（作者给的另一个选项）能缓解但治不了根：只要图片仍占 `60vh`，矮窗口（笔记本 768px 高）下同一个塌陷就会复现。

**实测佐证**（Headless Chrome，挂真实编译产物 `Server/static/assets/index-*.css` + 真实 `data-v-*` 作用域，`getBoundingClientRect()` 量；同一份 DOM 只回退/保留上表这 6+2 条规则）：

| 视口高 | 改前 图片 / 评论区 / 可见评论行 | 改后（`40vh`）图片 / 评论区 / 可见评论行 |
|---|---|---|
| 800px | 480 / **9px** / **0** | 320 / 167px / 5 |
| 668px（768 窗口） | 401 / **9px** / **0** | 267 / 110px / 3 |
| 600px（700 窗口） | 360 / **9px** / **0** | 240 / 110px / 3 |

改前的 `9px` 就是"看不见评论"的真相：只剩 `padding: 4px 0` 加 1px 上边框，一行都露不出来。改后 20 条评论时弹窗高度不变（692px，不撑高，#63），1000 字长帖下正文区先滚动、评论区仍是 110px。

### 14.2 帮助页文案（`views/HelpModal.vue`）

| 行 | 原文 | 改为 |
|---|---|---|
| `:7` | `赋能设计的 AIGC 系统——你说想法，它出图。三种玩法，照着说就行。` | `赋能设计的 AIGC 系统——你说想法，它出图。` |
| `:39` | `支持 jpg / png / webp / gif，≤10MB。每条消息都能带一张图。` | `支持 jpg / png / webp / gif，≤10MB。每条消息都能带图。` |

`:36` 的「✨ 以上三种只是基础玩法…」**保留不动**——删的是首行那句与它重复的「三种玩法，照着说就行」。

### 14.3 验收用例追加

| # | 操作 | 期望 |
|---|---|---|
| 61 | 打开一条**带图**且有评论的帖子 | 图片 ≤`40vh`；评论区可见（≥110px），评论与「发表」输入框**同屏**（视口高 ≥580px 时；更低时正文区滚动，评论区仍有 110px） |
| 62 | 把窗口压到 768px 高，重复 #61 | 评论区仍保有 110px 可滚动区，不被图片与输入框挤成 0 高；输入框同在弹窗内 |
| 63 | 打开一条有 20 条评论的**带图**帖 | 评论区在 `260px` 内滚动看到全部（#53 不变），弹窗不被撑高 |
| 64 | 打开「帮助」 | 首行无「三种玩法，照着说就行」；底部提示为「…每条消息都能带图。」 |
